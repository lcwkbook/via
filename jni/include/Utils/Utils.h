...
int sdkVersion = 0;
inline void recordTouchEvents(Rect &rect)
{
    std::thread([&] {
    // 启用触摸指针显示
    system("setenforce 0");
    system("su -c settings put system pointer_location 1");
    ImGuiIO &iooi = ImGui::GetIO();
    // 等待手指按下
    while (!iooi.MouseDown[0])
        usleep(1000);
    rect.startX = iooi.MousePos.x;
    rect.startY = iooi.MousePos.y;
    while (iooi.MouseDown[0])
    {
        rect.endX = iooi.MousePos.x;
        rect.endY = iooi.MousePos.y;
        usleep(1000);
    }
    // 禁用触摸指针显示
    system("su -c settings put system pointer_location 0");
    system("setenforce 1");
    }).detach();  // 新增：启动线程并分离
}
