...
namespace ImTricks {
	...
	namespace NotifyManager {
		...
		void AddNotify(const char* message, NotifyState state) {
			notifies.push_back({ message, state, static_cast<unsigned long long>(clock_gettime(CLOCK_MONOTONIC,NULL) + 3000) });
		}
		...
		void HandleNotifies(ImDrawList* draw) {
			...
			for (int i = 0; i < notifies.size(); i++)
			{
				...
				if (current_notify.time < static_cast<int>(clock_gettime(CLOCK_MONOTONIC,NULL)))
				{
