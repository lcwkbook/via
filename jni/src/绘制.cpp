...
void 绘制::更新对象数据()
{
    ...
    for (int a = 0; a < 世界数量; a++)
    {
        ...
        if (t_屏幕坐标.W > 0)
        {
            ...
            // 先判断是否是盒子类
            bool isBoxLike = (strstr(ClassName, "CharacterDeadInventoryBox_C") ||
            ...
            if (按钮.盒子物资 && isBoxLike && 对象信息.敌人信息.距离 <= 100)
            {
                float aimDistance = sqrtf(powf(PX - r_x, 2.0f) + powf(PY - r_y, 2.0f));
                if (aimDistance < 100.0f && 自瞄.瞄准目标 == -1)
                {
                    // 使用新偏移 0xD98
                    uintptr_t listBase = 读写.getPtr64(对象地址.敌人地址 + Offsets::BoxPickUpDataList);
                    int 盒内物资数量 = 0;
                    // 优先从 listBase+0x8 读取数量
                    ...
                    if (listBase != 0 && 盒内物资数量 > 0)
                    {
                        ...
                        for (int i = 0; i < 盒内物资数量; i++)
                        {
                            ...
                            if (name != "NULL" && name != "Error")
                            {
                                ...
                                // 描边
                                ImGui::GetBackgroundDrawList()->AddText(NULL, 30, {textPos.x - 1, textPos.y - 1}, ImColor(0, 0, 0, 255), name.c_str());
                                ImGui::GetBackgroundDrawList()->AddText(NULL, 30, {textPos.x + 1, textPos.y + 1}, ImColor(0, 0, 0, 255), name.c_str());
                                // 主体
